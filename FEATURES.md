...
- **Collapse animation is disabled** — phones get direct, instant node transforms instead of the d3-animated ones
- **Safari (Mac + iOS) drops card micro-transitions** — WebKit paints foreignObject HTML at the SVG origin when it uses `position`/`transform`/`transition` (WebKit bug 23113), so cards omit those properties in Safari; see "Safari foreignObject Workaround" in [Decisions](DECISIONS.md)
